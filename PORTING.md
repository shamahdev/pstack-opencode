...
bash scripts/sync-upstream.sh --pin 0.14.7   # diffs against the 0.14.7 tag/ref
bash scripts/sync-upstream.sh --ref main     # explicit ref
```

`--pin` overrides `--ref` and is the way to diff a pinned tag. A missing tag/ref fails loudly (the script uses `curl -f`).
