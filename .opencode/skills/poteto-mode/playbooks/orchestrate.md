...
- **Worker / verifier.** Runs with `agent: general` (use `agent: explore` for read-only verification). Give each worker its own worktree or branch when it writes. Briefs inline what workers need or point at repo paths. Prefer fewer, broader workers; one writer per worktree or branch (principle-separate-before-serializing-shared-state). Run a unit's verifier on a different model family from its worker.
...
- Never resume an agent to check on it; a resume restarts an idle agent. Probe read-only: the ledger, `units.tsv`, `gh`, pushed branches, the subagent's task status. Transcript mtime is not liveness.
