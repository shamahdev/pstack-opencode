...
3. Explore in subagents with `agent: "poteto-agent" (mode: subagent via Task tool)` and an explicit model per the Subagents section (the **guard-the-context-window** principle skill). Each returns file pointers, conventions, test commands, and entry points. No inlined dumps.
...
- [ ] Arm the 30-minute audit tick as a timed repeat loop. Never leave the cadence to memory.
...
- [ ] Run `/unslop` before each commit (`/deslop` only when installed) and `/no-comments` before review.
...
Each live lane runs at the PR head in its own worktree. Drive the real surface via the repo's `verify-*` skill or manual steps (`control-ui` / `control-cli` only when installed).
...
**Verify, live.** Tests alone are not sufficient verification. A PR is verified only when its unit, live, and perf boxes are all checked. Ten lanes on `xai/grok-4-fast` at the PR head, per the boot recipe.
