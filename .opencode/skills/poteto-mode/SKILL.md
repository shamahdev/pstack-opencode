...
---
> **Ported to opencode** from [`cursor/plugins/tree/main/pstack`](https://github.com/cursor/plugins/tree/main/pstack) v0.14.7 MIT. Model IDs mapped per `PORTING.md`. Use `skill("poteto-mode")` or `/poteto-mode` in opencode.
...
- Before commit → `/unslop` plus the **no-comments** skill (`/no-comments`). Where upstream prescribes `/deslop` from `cursor-team-kit` (not included here), treat it as optional and use `/unslop` when it is absent.
- Before review → the **no-comments** skill (`/no-comments`).
- Shipping UI / IDE / CLI → drive the real surface yourself (or via the repo's `verify-*` skill when one exists). Where upstream prescribes `control-cli` / `control-ui` from `cursor-team-kit` (not included here), use the opencode equivalent or manual live verification instead. For bug fixes, reproduce first on the same surface yourself; hand to the user only under the narrow Bug fix step 1 exception.
...
**Opencode port note.** Cursor Task fields `readonly`, `run_in_background`, and `environment` do not exist in opencode — use `agent: general` for full-access work and `agent: explore` for read-only work. They are informational where they still appear in quoted upstream references. Model roles are set per `setup-pstack`.

**Defaults for every `Task` call.** agent mode, file pointers not inlined context, explicit model per role (configurable via `/setup-pstack`; defaults `xai/grok-4-fast` for code, `anthropic/claude-opus-4-20250514` for prose and judgment). Code delegates tier by difficulty. The hardest changes (cross-cutting design, gnarly concurrency, subtle algorithms) go to your strongest judgment model (`anthropic/claude-opus-4-20250514`) when the task needs judgment or the intent is vague, and to your strongest instruction-following model (`anthropic/claude-opus-4-20250514`) when the work is a precisely specified sequence of steps to execute to the letter; trivial mechanical edits go to your fast code model. Per-role lines in the `/setup-pstack` rule override these defaults and the model choices in the routed skills (`how`, `why`, `arena`, `swarm`, `architect`, `interrogate`, `reflect`); a role with no line keeps its default, and a role line of `inherit-parent` or `auto` runs that role on the parent chat model (omit Task `model`).
