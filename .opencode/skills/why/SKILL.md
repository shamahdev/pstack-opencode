...
Before spawning investigators, list the available MCPs from the opencode environment. Use the available-tools map when present. Otherwise inspect the configured MCP servers for enabled MCP servers.
...
- `agent`: `general` (full access, MCPs enabled). Investigators still shouldn't write anything. That's a posture, not a sandbox.
- `model`: your configured why-investigators model (default `xai/grok-4-fast`)
...
- `agent`: `general` (full access, so citation spot-checks can use MCPs)
- `model`: your configured why-synthesizer model (default `anthropic/claude-opus-4-20250514`)
