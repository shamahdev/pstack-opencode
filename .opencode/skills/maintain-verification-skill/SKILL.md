...
0. **Locate the target.** Find the verification skill to maintain: the project-local skill whose body has launch/drive sections and a feature map (usually `.opencode/skills/verify-<app>/`; `.claude/skills/` is also scanned as a compat alias). Several candidates → ask which one; none → stop and point at `/create-verification-skill` instead of inventing a target.
